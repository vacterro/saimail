Source artifact: `saifren_live_20260917T151623Z.json`
...
Started 2026-09-17T15:16:23Z; 12 live calls of 6 planned, cap 24; stopped: no.

## LIVE_RUN_COUNT

- calls attempted: 6; returned content: 5; errors: 1
...
- S2.chain1: PASS; A via `deepseek/deepseek-v4-flash` said kind=hypothesis rung=unverified evidence_invented=False; transform ACCEPTED ; B via `MiniMaxAI/MiniMax-M3` evidence=NO inflation=False qualifier_retained=True next=COLLECT ARTIFACT; routes differ: True
...
- first targets ['B'], steps [['B', 'D', 'E']], routes ['MiniMaxAI/MiniMax-M3']
### control arm, n=0
- no measured calls
...
- run totals: EXACT 1, FALSE_IGNORE 1, OVER_OPEN 1, OTHER 1
- S3.mailbox via `MiniMaxAI/MiniMax-M3`: FAIL
  ...
  - M4: agent DEFER, deterministic selector IGNORE -> OTHER
  - EXACT 1, FALSE_IGNORE 1, OVER_OPEN 1, OTHER 1

## OPAQUE_CONTEXT_FINDINGS

- route `MiniMaxAI/MiniMax-M3`: 3 call(s); delta range 2879..2894 tokens (local prompt est 394..499, gateway prompt 3284..3393)
...
- S2.chain1:B via `MiniMaxAI/MiniMax-M3`: local_est=394, gateway_reported=3284, delta=2890
...
- S5.trial1:B_legacy via `MiniMaxAI/MiniMax-M3`: local_est=499, gateway_reported=3393, delta=2894
- S5.trial1:B_control via `deepseek/deepseek-v4-flash`: local_est=320, gateway_reported=3137, delta=2817

## ERRORS_TIMEOUTS

- S5.trial1:B_control EmptyOutput: no content; finish_reason=length
